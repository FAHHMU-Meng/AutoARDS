...
def train_one_epoch(data_loader, optimizer, model, epoch, scaler, cuda):
    ...
    for iteration, (data, gt, info) in enumerate(tqdm(data_loader, desc=f"Epoch {epoch}")):
        ...
        # B = data.shape[0]
        # data = rearrange(data, 'b c w h d -> c w h (b d)')
        # filtered = frangi_filter_gpu(data.squeeze(dim=0)).unsqueeze(dim=0)
        # data = rearrange(data, 'c w h (b d) -> b c w h d', b=B)
        # filtered = rearrange(filtered, 'c w h (b d) -> b c w h d', b=B)
        # data = torch.concat((data, filtered), dim=1).float()
...
def save_checkpoint(model, path, epoch):
    ...
    model_out_path = os.path.join(path, f"{epoch:03d}_PaCO2.pth")
