...
from CTCLIP.vector import VectorQuantize
from CTCLIP.attention import Attention, Transformer, ContinuousPositionBias
...
class CTViTEncoder(nn.Module):
    def __init__(
        ...
        ph, pw = self.patch_size
        self.patch_hw = (
            self.image_size[0] // ph,
            self.image_size[1] // pw,
        )

        # Relative position bias for attention
        self.spatial_rel_pos_bias = ContinuousPositionBias(dim=dim, heads=heads)

        # Convert video patches into embeddings
        self.to_patch_emb = nn.Sequential(
            Rearrange('b c (t pt) (h ph) (w pw) -> b t h w (c pt ph pw)',
                      pt=temporal_patch_size, ph=ph, pw=pw),
            nn.LayerNorm(channels * ph * pw * temporal_patch_size),
            nn.Linear(channels * ph * pw * temporal_patch_size, dim),
            nn.LayerNorm(dim)
        )
        ...
        self.enc_spatial_transformer = Transformer(depth=spatial_depth, **transformer_cfg)
        self.enc_temporal_transformer = Transformer(depth=temporal_depth, **transformer_cfg)
    ...
    @property
    def image_num_tokens(self):
        return self.patch_hw[0] * self.patch_hw[1]
    ...
    def encode(self, tokens):
        ...
        # Spatial transformer
        tokens = rearrange(tokens, 'b t h w d -> (b t) (h w) d')
        attn_bias = self.spatial_rel_pos_bias(h, w)
    ...
    def decode(self, tokens):
        ...
        # Temporal decoding
        tokens = rearrange(tokens, 'b t h w d -> (b h w) t d')
        tokens = self.dec_temporal_transformer(tokens, video_shape=video_shape, device=tokens.device())
        tokens = rearrange(tokens, '(b h w) t d -> b t h w d', b=b, h=h, w=w)
        ...
        # To pixels (implement this method separately)
        recon_video = self.to_pixels(tokens)
        return recon_video
    ...
    def calculate_video_token_mask(self, videos, video_frame_mask):
        ...
        first_mask = video_frame_mask[:, :1]
        rest_mask = video_frame_mask[:, 1:]

        rest_vq_mask = rearrange(rest_mask, 'b (f p) -> b f p', p=self.temporal_patch_size)
        frame_mask = torch.cat((first_mask, rest_vq_mask.any(dim=-1)), dim=-1)

        return repeat(frame_mask, 'b f -> b (f hw)', hw=(h // ph) * (w // pw))
    ...
    def forward(self, video):
        assert video.ndim in {4, 5}, "Expected video of shape [B, C, T, H, W] or [B, T, C, H, W]"
        tokens = self.to_patch_emb(video)
